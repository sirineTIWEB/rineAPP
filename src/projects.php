...
<section class="md:ml-12 ml-6 mb-20">
    ...
    <div class="flex gap-5 filter-btns">
        <article data-filter="*" class="flex justify-end bg-mylightblue w-28 md:w-52 md:h-24 h-14 pr-4">
            <h2 class="soustitre text-mybeige">All</h2>
        </article>
        <?php
...
<section
    ...
    if ($the_query->have_posts()) {
        while ($the_query->have_posts()) {
            ...
            <article data-filter="<?php echo esc_attr($slug); ?>"
                style="background-image: url('<?php echo get_template_directory_uri(); ?>/assets/images/mockups/rine.webp')"
                class="grid-item group relative mr-3 md:mr-9 h-[243px] w-40 md:w-[300px] overflow-hidden bg-cover bg-center transition-all duration-300 ease-in-out shrink-0 active:w-[170px] md:h-[455px] md:hover:w-96 float-left">
