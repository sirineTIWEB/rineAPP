...
<section class="md:h-[500px] h-[25vh] bg-myyellow dark:bg-mylightblue flex items-end md:mb-64 mb-32">
  ...
  <img class="absolute md:right-1/4 md:top-[28rem] top-80 right-24 h-16 md:h-auto dark:hidden"
    src="<?php echo get_template_directory_uri(); ?>/assets/icons/profile.svg" alt="gribouilli4">
  <img class="absolute md:right-1/4 md:top-[28rem] top-80 right-24 h-16 md:h-1/4 hidden dark:block"
    src="<?php echo get_template_directory_uri(); ?>/assets/icons/darkabout.svg" alt="gribouilli4">
</section>
<section class="flex flex-col items-center mb-16 md:mb-52">
  <h1 class="titre text-myyellow mb-6 md:mb-8 flex-wrap w-2/3 text-center">
    <?php esc_html_e('work experiences', 'rine2'); ?></h1>
  <div class="flex overflow-x-auto overflow-y-hidden max-w-full pl-6 md:pl-12 pb-3">
    <article
      class="group relative mr-3 md:mr-9 h-[243px] w-40 md:w-[300px] overflow-hidden bg-[url('../assets/images/mockups/rine.webp')] bg-cover bg-center transition-all duration-300 ease-in-out shrink-0 active:w-[170px] md:h-[455px] md:hover:w-96">
...
<section class="relative flex justify-end h-[80vh] p-12 md:mb-60 mb-20">
  <div
    class="absolute inset-0 h-[720px] w-full bg-[linear-gradient(to_right,#93B5C9_1.5px,transparent_1.5px),linear-gradient(to_bottom,#93B5C9_1.5px,transparent_1.5px)] bg-[size:30px_30px] z-[-1] border-b-[1.5px] border-mylightblue">
  </div>
  <!-- Contenu de la section -->
  <h1 class="titre text-mydarkblue dark:text-mybeige bg-mybeige dark:bg-mydarkblue h-fit w-fit">
    <?php esc_html_e('My skills', 'rine2'); ?></h1>
  <img src="<?php echo get_template_directory_uri(); ?>/assets/icons/skills.svg" alt="gribouillis"
    class="absolute h-10 md:h-auto left-1/3 bottom-0">
...
<section class="w-full h-[50vh] flex flex-col md:flex-row md:mb-56 mb-20">
  ...
  if ($random_project->have_posts()) {
    while ($random_project->have_posts()) {
      $random_project->the_post();
      ?>

      <div
        style="background-image: url('<?php echo get_template_directory_uri(); ?>/assets/images/mockups/rine.webp')"
        class=" h-1/2 md:h-auto md:w-1/2 flex flex-col justify-end items-end pr-6 md:pb-40 pb-6 bg-moiPC bg-cover bg-center">
        <h2 class="soustitre text-myyellow"> <?php the_title() ?> </h2>
        <p class="texte text-myyellow"><?php echo get_field('date'); ?></p>
      </div>
  ...
  <div class="h-1/2 md:h-auto md:w-1/2 bg-mydarkblue dark:bg-myyellow flex flex-col items-start justify-end pl-6">
    <h1 class="soustitre md:w-1/2 w-2/3 text-mybeige dark:text-mylightblue text-end pt-5 sm:pt-0">
      <?php printf(__('Want  to see <br> more <span class="titre">PROJECTS?</span>', 'rine2')); ?></h1>
