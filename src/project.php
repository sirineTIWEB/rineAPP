...
<section class="bg-myyellow dark:bg-mydarkblue py-0 md:py-16">
    ...
    <?php
    // Get the project slug from URL parameter
    $project_slug = isset($_GET['slug']) ? sanitize_title($_GET['slug']) : '';

    // Get the WordPress post by slug
    if ($project_slug) {
        $args = array(
            'name' => $project_slug,
            'post_type' => 'project',
        ...
        $projet = get_posts($args);
        if ($projet && !empty($projet)) {
            global $post;
            $post = $projet[0];
            setup_postdata($post);
        }
            ...
            <div class="embla__viewport">
                <div class="embla__container">
                    <div class="embla__slide">
                        <div
                            class="flex h-96 bg-white rounded-lg dark:bg-neutral-900 shadow-md">
                            <img class="object-cover" src="<?php echo get_the_post_thumbnail_url(); ?>" alt="<?php esc_html_e('Project\'s mock-up', 'rine2'); ?>">
                        </div>
                    </div>
                    <?php
